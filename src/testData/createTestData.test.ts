  ...
  it('create ' + entries + ' entries', () => {
    return new Promise<void>(done => {
      ...
      for (let i = 0; i < entries; i++) {
        ...
        setTimeout(async () => {
          await callServer(undefined, `user=xx&lat=${lat}&lon=${lon}&timestamp=R3Pl4C3&hdop=${Math.floor(Math.random() * 15) + 1}&altitude=${i+1}&speed=${39 + i*2.5}&heading=${262 + Math.floor(Math.random() * 20) - 10}&eta=${eta}&eda=${(6.94*1000  * ((entries - 1 - i) / (entries - 1))  ).toFixed(1)}&key=${key}`, 200, "GET");
